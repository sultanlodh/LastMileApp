...
class KafkaConsumer:
    def __init__(self, consumer_settings, consumer_topic, dest_settings, source_connection,
                 ...
                 Consignment_History_Final_Schema, batch_size=1, max_workers=1):
        ...
        self.foc_consigments = 'fact_consignment_lm'
        ...
        try:
            while True:
                ...
                if msg.value() is None:
                    logging.warning("Received a message with no value. Skipping...")
                    continue
    ...
    def handle_error(self, error):
        if error.code() == KafkaError._PARTITION_EOF:
            logging.info(f"Reached the end of partition: {error}")
            return
        elif error.code() == KafkaError._ALL_BROKERS_DOWN:
            logging.critical("All brokers are down. Check the Kafka cluster status.")
        elif error.is_fatal():
            logging.critical(f"Fatal error encountered: {error}")
            raise Exception(f"Fatal error in Kafka consumer: {error}")
        else:
    ...
    def history_table_custom_updates(self, foc_consigments_tbl, table_name, df, message_value):
        try:
            ...
            if rows_updated > 0:
                ...
                update_query = f"""update {foc_consigments_tbl} 
                                   set ModifiedTs =  '{ModifiedTs}', ModifiedBy = '{ModifiedBy}' 
                                   where foc_cnno = '{foc_cnno}';"""
                dest_cursor.execute(update_query)
